 ...
 * Plugin URI:  https://wordpress.org/plugins/quran-translations-by-edc/
...
$code .= '<li id="element-'.$suranid.'" class="surah-li" data-id="'.$suranid.'" data-surah-name="'.str_replace("&apos;","`", $surhname).'" data-audio-url="'.$arrayname[$i][1].'">'.$nl;
$code .= $suranid.'- '.$surhname.$nl;
$code .= '</li>'.$nl;
}
$code .= '</ul>'.$nl;
$code .= '</div>'.$nl;
...
function quran_playlist_options() {
  ...
  if(isset($_POST['submitted']) && $_POST['submitted'] == 1){
  	...
  	update_option( 'playlist_title', sanitize_text_field($_POST['playlist_title']) );
  }
...
?>
	...
	<!--
					...
					<div>
						<input id="playlist_title" type="text" name="playlist_title" value="<?php echo esc_html($playlist_title); ?>" />
